// セクションの識別アイコン（lucide由来の線画SVG・currentColor）。
// 絵文字をUIに直接使わず（機種依存・テンプレ感の主因）、識別に効くSVGアイコンに統一する
// （O-2-8・2026-08-15。gajumaru-info の section-icons.ts と同じパターン）。
// キー名は src/data/sections.ts の icon フィールドと一致させること。
